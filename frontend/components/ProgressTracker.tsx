...
export default function ProgressTracker({
  ...
  useEffect(() => {
    ...
    if (progress?.status === "running") {
      ...
      intervalRef.current = setInterval(() => {
        ...
        const isActive =
          timerActiveRef.current && currentProgress?.status === "running";
  ...
  useEffect(() => {
    ...
    const fetchInitialProgress = async (): Promise<ProgressData | null> => {
      try {
        ...
        if (response.status === 404) {
          ...
          setError(
            "Trip not found. The trip may have expired or the server was restarted. Please start a new trip."
          );
    ...
    const eventSource = new EventSource(
      `${apiUrl}/api/trips/${tripId}/progress/stream`
    );
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (data.debug) {
          ...
          console.log(
            `📍 Step: ${data.current_step || 0}/${data.total_steps || 3}`
          );
          if (data.debug.elapsed_time !== undefined) {
            console.log(
              `⏱️  Elapsed: ${data.debug.elapsed_time}s | Remaining: ~${
                data.debug.remaining_time || 0
              }s`
            );
          }
          if (data.debug.all_tasks) {
            ...
            data.debug.all_tasks.forEach((task, idx) => {
              console.log(
                `  ${idx + 1}. ${task.task} → ${task.agent} [${task.status}]`
              );
            });
          }
          console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data.debug) {
            console.log(
              "%c🔍 Agent Debug Info (Polling)",
              "color: #3b82f6; font-weight: bold; font-size: 14px;"
            );
            console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
            console.log(`📋 Task: ${data.debug.task_name || "N/A"}`);
            console.log(
              `👤 Assigned to: ${data.debug.assigned_agent || "N/A"}`
            );
            console.log(`📊 Status: ${data.debug.agent_status || "N/A"}`);
            console.log(
              `📍 Step: ${data.current_step || 0}/${data.total_steps || 3}`
  ...
  const agents: AgentStatus[] = [
    {
      ...
      icon: "🔍",
      status:
        progress.current_step === undefined || progress.current_step === 0
          ? "waiting"
    ...
    },
    {
      ...
      icon: "⭐",
      status:
        progress.current_step === undefined || progress.current_step < 2
          ? "waiting"
    ...
    },
    {
      ...
      icon: "📋",
      status:
        progress.current_step === undefined || progress.current_step < 3
          ? "waiting"
          : progress.status === "completed"
          ? "complete"
      ...
      <div className="text-center">
        ...
        {progress.status === "running" && (
          <p className="text-sm text-gray-600 mb-4">
            ...
            {agents.find((a) => a.step === currentStep)?.name || "Processing"}
            ...
          </p>
        )}
        {progress.status === "completed" && (
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              <p className="text-sm font-medium text-blue-900">
                Step {currentStep}/{totalSteps}:{" "}
                {agents.find((a) => a.id === progress.current_agent)?.name}
              </p>
            ...
            progress.estimated_time_remaining > 0 && (
              <div className="mt-2 flex items-center space-x-2 text-xs text-blue-600">
                ...
                <span>
                  ...
                  {agents
                    .find((a) => a.step === currentStep)
                    ?.name.split(" ")[1] || "Processing"}{" "}
                  | Est. total:{" "}
                  {Math.ceil((progress.estimated_time_remaining || 0) / 60)}-
                  {Math.ceil((progress.estimated_time_remaining || 0) / 60) + 1}{" "}
                  min
                </span>
              </div>
            )}
